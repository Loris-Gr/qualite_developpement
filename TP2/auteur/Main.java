class Main {
  public static void main(String[] args) {
    ...
    Auteur aristophane = new Auteur("Aristophane", 
                                    ...
                                    100, citation_guêpes,
                                    0, "");
    assert sophocle.getQualiteTragedie() == 95 ;
    assert sophocle.getCitationTragedie().equals(citation_antigone) ;
    assert euripide.getQualiteTragedie() == 92 ;
    assert euripide.getCitationTragedie().equals(citation_hécube) ;
    assert aristophane.getQualiteTragedie() == 5 ;
    assert aristophane.getCitationTragedie().equals(citation_thesmophories) ;

    assert sophocle.getQualiteComedie() == 0 ;
    assert sophocle.getCitationComedie().equals("") ;
    assert euripide.getQualiteComedie() == 0 ;
    assert euripide.getCitationComedie().equals("") ;
    assert aristophane.getQualiteComedie() == 100 ;
    assert aristophane.getCitationComedie().equals(citation_guêpes) ;

    assert sophocle.getQualiteDrame() == 60 ;
    assert sophocle.getCitationDrame().equals(citation_limiers) ;
    assert euripide.getQualiteDrame() == 80 ;
    assert euripide.getCitationDrame().equals(citation_cyclope) ;
    assert aristophane.getQualiteDrame() == 0 ;
    assert aristophane.getCitationDrame().equals("") ;

    assert sophocle.pointFort().equals ("Tragédie") ;
    ...
    assert euripide.citationStyle(Style.TRAGEDIE).equals(citation_hécube) ;
    assert aristophane.citationStyle(Style.COMEDIE).equals(citation_guêpes) ;
    assert aristophane.citationStyle(Style.TRAGEDIE).equals(citation_thesmophories) ;
  }
}